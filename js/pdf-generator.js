...
async function criarDocumentoPDF(dados, protocolo, logos) {
  ...
  if (logos.logo1) doc.addImage(logos.logo1, "JPEG", margin, 3, 18, 18)
  if (logos.logo2) doc.addImage(logos.logo2, "JPEG", pageWidth - margin - 18, 3, 18, 18)
...
// ===============================
// ABRIR PDF (DESKTOP) / BAIXAR (MOBILE)
// ===============================
function openPDFInNewWindow(dados, protocolo) {
  carregarLogos().then(async (logos) => {
    ...
    const filename = makePdfFilename(dados, protocolo)

    // 📱 MOBILE → DOWNLOAD / VISUALIZADOR NATIVO
    if (isMobile()) {
