...
function generatePDF(dados, protocolo) {
  ...
  y += 6
  doc.text(`CPF: ${formatCPF(dados.cpf || "")}`, margin, y)
  ...
  doc.setTextColor(0, 0, 0)
  doc.setFont("helvetica", "normal")
  const solicitacoes = dados.solicitacao || []
  ...
  doc.setTextColor(0, 0, 0)
  doc.setFont("helvetica", "normal")
  const ocorrencias = dados.ocorrencia || []
  const ocorrenciasText = ocorrencias.length > 0 ? ocorrencias.join(", ") : "-"
  const ocorrenciasLines = doc.splitTextToSize(ocorrenciasText, contentWidth)
  ...
  doc.setTextColor(0, 0, 0)
  doc.setFont("helvetica", "normal")
  const patologias = dados.patologia || []
  const patologiasText = patologias.length > 0 ? patologias.join(", ") : "-"
  const patologiasLines = doc.splitTextToSize(patologiasText, contentWidth)
  ...
  doc.setTextColor(0, 0, 0)
  doc.setFont("helvetica", "normal")
  const locais = dados.local_anomalia || []
  const locaisText = locais.length > 0 ? locais.join(", ") : "-"
  ...
  y += 5
  doc.setFont("helvetica", "normal")
  const descricaoLines = doc.splitTextToSize(dados.descricao_situacao || "-", contentWidth)
  ...
  y += 5
  doc.setFont("helvetica", "normal")
  const analiseLines = doc.splitTextToSize(dados.analise_tecnica || "-", contentWidth)
  ...
  y += 5
  doc.setFont("helvetica", "normal")
  const recomendacoesLines = doc.splitTextToSize(dados.recomendacoes || "-", contentWidth)
  ...
  y += 5
  doc.setFont("helvetica", "normal")
  const parecerLines = doc.splitTextToSize(dados.parecer_final || "-", contentWidth)
...
function downloadPDF(dados, protocolo) {
  const doc = generatePDF(dados, protocolo)
